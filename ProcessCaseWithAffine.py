...
for nid in needleImageIds:
  ...
  if not os.path.isfile(fixedMask):
    ...
    # affine
    BFRegister(fixed=fixedImage,moving=movingImage,movingMask=movingMask,fixedMask=fixedMask,affineTfm=affineTfm,initTfm=rigidTfm,log=log)
    # bspline
    print 'latest BSpline transform path in else case BEFORE RUNNING the BSPLINE: '+str(bsplineTfm)
    BFRegister(fixed=fixedImage,moving=movingImage,movingMask=movingMask,fixedMask=fixedMask,bsplineTfm=bsplineTfm,initTfm=affineTfm,log=log)
